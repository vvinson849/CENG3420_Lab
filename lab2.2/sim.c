...
void handle_slli(unsigned int cur_inst) {
    unsigned int rd = MASK11_7(cur_inst), rs1 = MASK19_15(cur_inst);
    int imm = sext(MASK24_20(cur_inst), 5);
    NEXT_LATCHES.REGS[rd] = CURRENT_LATCHES.REGS[rs1] << imm;
}
...
void handle_srli(unsigned int cur_inst) {
    unsigned int rd = MASK11_7(cur_inst), rs1 = MASK19_15(cur_inst);
    int imm = sext(MASK24_20(cur_inst), 5), mask = 0;
    for(int i=0; i<(12-imm); i++)
        mask = (mask<<1) + 1;
    NEXT_LATCHES.REGS[rd] = ((CURRENT_LATCHES.REGS[rs1]>>imm) & mask);
}
...
void handle_lui(unsigned int cur_inst) {
    ...
    NEXT_LATCHES.REGS[rd] = (sext(MASK31_12(cur_inst), 20) << 12);
}
...
void handle_srl(unsigned int cur_inst) {
    unsigned int rd = MASK11_7(cur_inst),
        ...
        rs2 = MASK24_20(cur_inst);
    int  mask = 0;
    for(int i=0; i<(32-CURRENT_LATCHES.REGS[rs2]); i++)
        mask = (mask<<1) + 1;
    NEXT_LATCHES.REGS[rd] = ((CURRENT_LATCHES.REGS[rs1] >> CURRENT_LATCHES.REGS[rs2]) & mask);
}
...
void handle_lh(unsigned int cur_inst) {
    unsigned int rd = MASK11_7(cur_inst), rs1 = MASK19_15(cur_inst);
    int imm12 = MASK31_20(cur_inst), base = sext(imm12, 12) + CURRENT_LATCHES.REGS[rs1];
    NEXT_LATCHES.REGS[rd] = (MEMORY[base+1]<<8) + (MEMORY[base]);
}

void handle_lw(unsigned int cur_inst) {
    unsigned int rd = MASK11_7(cur_inst), rs1 = MASK19_15(cur_inst);
    int imm12 = MASK31_20(cur_inst), base = sext(imm12, 12) + CURRENT_LATCHES.REGS[rs1];
    NEXT_LATCHES.REGS[rd] = (MEMORY[base+3]<<24) + (MEMORY[base+2]<<16) + (MEMORY[base+1]<<8) + (MEMORY[base]);
}


void handle_sb(unsigned int cur_inst) {
    unsigned int rs2 = MASK19_15(cur_inst), rs1 = MASK24_20(cur_inst);
    int imm12 = MASK11_7(cur_inst)+(MASK31_25(cur_inst)<<5), base = sext(imm12, 12) + CURRENT_LATCHES.REGS[rs2];
    MEMORY[base] = MASK7_0(CURRENT_LATCHES.REGS[rs1]);
}
...
void handle_sh(unsigned int cur_inst) {
    ...
    int imm12 = MASK11_7(cur_inst)+(MASK31_25(cur_inst)<<5), base = sext(imm12, 12) + CURRENT_LATCHES.REGS[rs2];
    MEMORY[base] = MASK7_0(CURRENT_LATCHES.REGS[rs1]);
    MEMORY[base+1] = MASK15_8(CURRENT_LATCHES.REGS[rs1]);
}
...
void handle_sw(unsigned int cur_inst) {
    ...
    MEMORY[base] = MASK7_0(CURRENT_LATCHES.REGS[rs1]);
    MEMORY[base+1] = MASK15_8(CURRENT_LATCHES.REGS[rs1]);
    MEMORY[base+2] = MASK23_16(CURRENT_LATCHES.REGS[rs1]);
    MEMORY[base+3] = MASK31_24(CURRENT_LATCHES.REGS[rs1]);
}
